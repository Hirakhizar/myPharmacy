            ...
            <div class="container form-container">
                <div class="row mt-3">
                    <div class="col-md-12 mb-3 form-group" style="text-align: center">
                        <h2 class="text" style="color:#7172b9;">Update Expence</h2>
                        <p class="text-muted">Update your expnece here</p>
                    </div>
                </div>
                <div class="card my-5 w-100">
                    <div class="card-body" style="background-color: #f1f1f1; border-radius: 8px; padding: 20px; border: 1px solid #ccc; margin: 20px">
                       
                        <form action="{{ url('expenses/update',['id'=>$expense->id]) }}" method="POST">
                            @csrf
                            <div class="row mt-3">
                                <div class="col-md-4 form-group">
                                    ...
                                    <select class="form-control" id="method" name="category" required>
                                        ...
                                        <option value="{{ $category->id }}"{{$category->name== $expense->category->name ?'selected' : '' }}>{{ $category->name }}</option>
                                        @endforeach
                                ...
                                <div class="col-md-4 form-group">
                                    ...
                                    <select class="form-control" id="method" name="subCategory" required>
                                        ...
                                        <option value="{{ $category->id }}" {{$category->name== $expense->subcategory->name ?'selected' : '' }}>{{ $category->name }}</option>
                                        @endforeach
                                ...
                                <div class="col-md-4 form-group">
                                    ...
                                    <input type="date" class="form-control"  name="date" value={{ $expense->date }} placeholder="" required>
                                </div>
                                <div class="col-md-4 form-group">
                                    <label for="amount"><b style="color:#7172b9;">Amount</b></label>
                                    <input type="number" class="form-control" id="amount" name="amount" value={{ $expense->amount }} placeholder="" required>
                                </div>
                                <div class="col-md-4 form-group">
                                    <label for="date"><b style="color:#7172b9;">Description</b></label>
                                    <input type="text" class="form-control" id="dateInput" name="head" value={{ $expense->description }} placeholder="" required>
                                </div>
